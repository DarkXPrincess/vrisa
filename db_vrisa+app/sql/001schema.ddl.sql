...
CREATE TABLE institucion (
    ...
    -- validación por parte del admin del sistema
    estado estado NOT NULL DEFAULT 'activo',
...
CREATE TABLE usuario_institucion (
    ...
    CONSTRAINT fk_ui_institucion FOREIGN KEY (id_i) REFERENCES institucion (id_i),
    CONSTRAINT fk_ui_usuario FOREIGN KEY (id_u) REFERENCES usuario (id_u)
);
CREATE TABLE estacion (
    ...
    -- Coordenada latitud (-90 a 90)
    longitud NUMERIC(10, 7),
    -- Coordenada longitud (-180 a 180)
    geom GEOGRAPHY(Point, 4326),
    -- Generado automáticamente por trigger
    id_i INT,
    -- FK a institución propietaria
    e_validacion validacion NOT NULL DEFAULT 'en_espera',
    estado estado NOT NULL DEFAULT 'activo',
    CONSTRAINT fk_estacion_institucion FOREIGN KEY (id_i) REFERENCES institucion (id_i)
);
CREATE TABLE usuario_estacion (
    ...
    CONSTRAINT fk_ue_usuario FOREIGN KEY (id_u) REFERENCES usuario (id_u),
    CONSTRAINT fk_ue_estacion FOREIGN KEY (id_e) REFERENCES estacion (id_e)
);
...
CREATE TABLE sensor (
    ...
    CONSTRAINT fk_sensor_estacion FOREIGN KEY (id_e) REFERENCES estacion (id_e)
);
CREATE TABLE sensor_variable (
    ...
    CONSTRAINT fk_sv_sensor FOREIGN KEY (id_sen) REFERENCES sensor (id_sen),
    CONSTRAINT fk_sv_variable FOREIGN KEY (id_var) REFERENCES variable (id_var)
);